...
$types = array(
    'charizard' => array(
        "query" => "SELECT COUNT(*) AS count FROM MODULE_UNDERGROUND WHERE (grade_A = 'Science' AND grade_B = 'Science' AND grade_C = 'Science' AND grade_D = 'Science')",
        "color" => "red",
        "note" => "4 Science grade CCDs"
    ),
    'charmeleon' => array(
        "query" => "SELECT COUNT(*) AS count FROM MODULE_UNDERGROUND WHERE ((grade_A = 'Science') + (grade_B = 'Science') + (grade_C = 'Science') + (grade_D = 'Science')) = 3",
        "color" => "orange",
        "note" => "3 Science grade CCDs"
    ),
    'charmander' => array(
        "query" => "SELECT COUNT(*) AS count FROM MODULE_UNDERGROUND WHERE ((grade_A = 'Science') + (grade_B = 'Science') + (grade_C = 'Science') + (grade_D = 'Science')) BETWEEN 1 AND 2",
        "color" => "yellow",
...
foreach ($types as $type => $details) {
    ...
    if ($result) {
        ...
        // Add to good_module_underground_count if it's a Charizard, Charmander, or Chameleon
        if (in_array($type, ['charizard', 'charmeleon', 'charmander'])) {
            $good_module_underground_count += $count;
        }
    } else {
...
$query = "SELECT COUNT(*) FROM MODULE_UNDERGROUND WHERE ID >= 1";
$result = mysql_query($query);
$row = mysql_fetch_row($result);
$total_module_undergrounds = $row ? $row[0] : 0;
echo ('<TD align="left">'); echo ($total_module_undergrounds); echo ('</TD>');

// Calculate and display yield
$yield = $total_module_undergrounds ? $good_module_underground_count / $total_module_undergrounds : 0;
