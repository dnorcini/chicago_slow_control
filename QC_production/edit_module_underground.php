...
if (isset($_POST['id'])) {
    ...
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0777, true)) {
            module_underground('Failed to create directory: ' . $upload_dir);
        }
    }
    ...
    if (!empty($query_parts)) {
        ...
        if (!$result) {
            ...
            module_underground('Query failed: ' . mysql_error());
        }
    }
}
...
<?php echo "<b>Image 2, High Temp - [1 skip, 1x1 binning, 30rx6400c, Serial register, 10s Exposure] - Aim: Serial Register Defect</b>"; ?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <table border="1">
	<tr>
...
<?php echo "<b>Image 3, High Temp - [1000 skip, 1x1 binning, 30rx640c, Serial register, 0s Exposure] - Aim: Single Electron Resolution</b>"; ?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <table border="1">
	<tr>
	...
	<tr>
		...
		if (file_exists($image3_high_file_path) && !isset($_SESSION['file_url_' . $module_underground_id]['image3_high_file'])) {
		    $_SESSION['file_url_' . $module_underground_id]['image3_high_file'] = $base_url . $image3_high_file_name;  // Use base_url for the web link
		}
		...
		if (!empty($image3_high_file_name) && file_exists($image3_high_file_path)) {
		    // File exists, keep the session variables and show the icon
		    $file_exists = true;
		    $file_url = $_SESSION['file_url_' . $module_underground_id]['image3_high_file'];
		} else {
		    // File does not exist, clear the session variables and remove the icon
		    $file_exists = false;
		}
                ...
                if (file_exists($image3_high_log_path) && !isset($_SESSION['log_url_' . $module_underground_id]['image3_high_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image3_high_log'] = $base_url . $image3_high_log_name;  // Use base_url for the web link
                }
		...
		// Check if the log exists on the server (log system path)
                if (!empty($image3_high_log_name) && file_exists($image3_high_log_path)) {
                    // Log exists, keep the session variables and show the icon
                    $log_exists = true;
                    $log_url = $_SESSION['log_url_' . $module_underground_id]['image3_high_log'];
                } else {
                    // Log does not exist, clear the session variables and remove the icon
                    $log_exists = false;
                }
...
<!-- Image 31 - Single Electron Resolution -->
<?php echo "<b>Image 3-1, Low Temp (while cooling)- [500 skip, 10x1 binning, 30rx640c, Active region, 3s Exposure] - Aim: Single Electron Resolution, Gain</b>"; ?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <table border="1">
        <tr>
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image31_low_defects_' . $amp, $yes_no_blank_array, ${'image31_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image31_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image31_low_cti_visual_' . $amp}); ?></td>
            <td><input type="text" name="image31_low_peak1_<?php echo $amp; ?>" value="<?php echo ${'image31_low_peak1_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image31_low_peak2_<?php echo $amp; ?>" value="<?php echo ${'image31_low_peak2_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image31_low_sigma_<?php echo $amp; ?>" value="<?php echo ${'image31_low_sigma_' . $amp}; ?>" size="15"></td>
            <td><?php generate_dropdown('image31_low_front_' . $amp, $yes_no_blank_array, ${'image31_low_front_' . $amp}); ?></td>
            <td><input type="text" name="image31_low_comments_<?php echo $amp; ?>" value="<?php echo ${'image31_low_comments_' . $amp}; ?>" size="40"></td>
            <td><input type="text" name="image31_low_reference_<?php echo $amp; ?>" value="<?php echo ${'image31_low_reference_' . $amp}; ?>" size="50"></td>
        </tr>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image31_low_log_path'} = $upload_dir . ${'image31_low_log_name'};

                if (file_exists(${'image31_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image31_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image31_low_file'] = $base_url . ${'image31_low_file_name'};
                }
                ...
                if (file_exists(${'image31_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image31_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image31_low_log'] = $base_url . ${'image31_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 31_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                ...
                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 31_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
...
<?php echo "<b>Image 3-2, Low Temp (after temp stablized) - [500 skip, 10x1 binning, 30rx640c, Active region, 3s Exposure] - Aim: Dark Current</b>"; ?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <table border="1">
        <tr>
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image32_low_defects_' . $amp, $yes_no_blank_array, ${'image32_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image32_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image32_low_cti_visual_' . $amp}); ?></td>
            <td><input type="text" name="image32_low_peak1_<?php echo $amp; ?>" value="<?php echo ${'image32_low_peak1_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image32_low_peak2_<?php echo $amp; ?>" value="<?php echo ${'image32_low_peak2_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image32_low_sigma_<?php echo $amp; ?>" value="<?php echo ${'image32_low_sigma_' . $amp}; ?>" size="15"></td>
            <td><?php generate_dropdown('image32_low_front_' . $amp, $yes_no_blank_array, ${'image32_low_front_' . $amp}); ?></td>
            <td><input type="text" name="image32_low_comments_<?php echo $amp; ?>" value="<?php echo ${'image32_low_comments_' . $amp}; ?>" size="40"></td>
            <td><input type="text" name="image32_low_reference_<?php echo $amp; ?>" value="<?php echo ${'image32_low_reference_' . $amp}; ?>" size="50"></td>
        </tr>
        <?php $count++; $count_plus++; endforeach; ?>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image32_low_log_path'} = $upload_dir . ${'image32_low_log_name'};

                if (file_exists(${'image32_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image32_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image32_low_file'] = $base_url . ${'image32_low_file_name'};
                }
                ...
                if (file_exists(${'image32_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image32_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image32_low_log'] = $base_url . ${'image32_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 32_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                ...
                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 32_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
...
<!-- Image 4 -  -->
<?php echo "<b>Image 4, Low Temp - [1 skip, 1x1 binning, 1600rx6400c, Active region, 500s Exposure] - Aim: Measure Noise, Sharpness of Tracks, CTI, Defect Map</b>"; ?>
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    <input type="hidden" name="id" value="<?php echo $id; ?>">
    <table border="1">
        <tr>
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image4_low_defects_' . $amp, $yes_no_blank_array, ${'image4_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image4_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image4_low_cti_visual_' . $amp}); ?></td>
            ...
            <td><?php generate_dropdown('image4_low_front_' . $amp, $yes_no_blank_array, ${'image4_low_front_' . $amp}); ?></td>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image4_low_log_path'} = $upload_dir . ${'image4_low_log_name'};

                if (file_exists(${'image4_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image4_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image4_low_file'] = $base_url . ${'image4_low_file_name'};
                }
                if (!empty(${'image4_low_file_name'}) && file_exists(${'image4_low_file_path'})) {
                    $file_exists = true;
                    $file_url = $_SESSION['file_url_' . $module_underground_id]['image4_low_file'];
                } else {
                    $file_exists = false;
                }

                if (file_exists(${'image4_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image4_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image4_low_log'] = $base_url . ${'image4_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 4_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                ...
                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 4_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    ...
    <table border="1">
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image5_low_defects_' . $amp, $yes_no_blank_array, ${'image5_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image5_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image5_low_cti_visual_' . $amp}); ?></td>
            <td><input type="text" name="image5_low_peak1_<?php echo $amp; ?>" value="<?php echo ${'image5_low_peak1_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image5_low_peak2_<?php echo $amp; ?>" value="<?php echo ${'image5_low_peak2_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image5_low_sigma_<?php echo $amp; ?>" value="<?php echo ${'image5_low_sigma_' . $amp}; ?>" size="15"></td>
            <td><?php generate_dropdown('image5_low_front_' . $amp, $yes_no_blank_array, ${'image5_low_front_' . $amp}); ?></td>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image5_low_log_path'} = $upload_dir . ${'image5_low_log_name'};

                if (file_exists(${'image5_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image5_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image5_low_file'] = $base_url . ${'image5_low_file_name'};
                }
                if (!empty(${'image5_low_file_name'}) && file_exists(${'image5_low_file_path'})) {
                    $file_exists = true;
                    $file_url = $_SESSION['file_url_' . $module_underground_id]['image5_low_file'];
                } else {
                    $file_exists = false;
                }

                if (file_exists(${'image5_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image5_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image5_low_log'] = $base_url . ${'image5_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 5_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                ...
                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 5_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    ...
    <table border="1">
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image6_low_defects_' . $amp, $yes_no_blank_array, ${'image6_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image6_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image6_low_cti_visual_' . $amp}); ?></td>
            <td><input type="text" name="image6_low_peak1_<?php echo $amp; ?>" value="<?php echo ${'image6_low_peak1_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image6_low_peak2_<?php echo $amp; ?>" value="<?php echo ${'image6_low_peak2_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image6_low_sigma_<?php echo $amp; ?>" value="<?php echo ${'image6_low_sigma_' . $amp}; ?>" size="15"></td>
            <td><?php generate_dropdown('image6_low_front_' . $amp, $yes_no_blank_array, ${'image6_low_front_' . $amp}); ?></td>
            <td><input type="text" name="image6_low_comments_<?php echo $amp; ?>" value="<?php echo ${'image6_low_comments_' . $amp}; ?>" size="40"></td>
            <td><input type="text" name="image6_low_reference_<?php echo $amp; ?>" value="<?php echo ${'image6_low_reference_' . $amp}; ?>" size="50"></td>
        </tr>
        <?php $count++; $count_plus++; endforeach; ?>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image6_low_log_path'} = $upload_dir . ${'image6_low_log_name'};

                if (file_exists(${'image6_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image6_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image6_low_file'] = $base_url . ${'image6_low_file_name'};
                }
                if (!empty(${'image6_low_file_name'}) && file_exists(${'image6_low_file_path'})) {
                    $file_exists = true;
                    $file_url = $_SESSION['file_url_' . $module_underground_id]['image6_low_file'];
                } else {
                    $file_exists = false;
                }

                if (file_exists(${'image6_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image6_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image6_low_log'] = $base_url . ${'image6_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 6_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                <label for="image6_low_file">Image File:</label>
                <input type="file" name="image6_low_file" accept="image/png, image/jpeg, application/pdf">

                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 6_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data">
    ...
    <table border="1">
        ...
        <tr>
            <td><?php echo "ch" . $count . " (ext" . $count_plus . ")"; ?></td>
            <td><?php generate_dropdown('image7_low_defects_' . $amp, $yes_no_blank_array, ${'image7_low_defects_' . $amp}); ?></td>
            <td><?php generate_dropdown('image7_low_cti_visual_' . $amp, $yes_no_blank_array, ${'image7_low_cti_visual_' . $amp}); ?></td>
            <td><input type="text" name="image7_low_peak1_<?php echo $amp; ?>" value="<?php echo ${'image7_low_peak1_' . $amp}; ?>" size="15"></td>
            <td><input type="text" name="image7_low_peak2_<?php echo $amp; ?>" value="<?php echo ${'image7_low_peak2_' . $amp}; ?>" size="15"></td>
        ...
        <tr>
            <td align="left" colspan="9" style="border: none; white-space: nowrap;">
                ...
                ${'image7_low_log_path'} = $upload_dir . ${'image7_low_log_name'};

                if (file_exists(${'image7_low_file_path'}) && !isset($_SESSION['file_url_' . $module_underground_id]['image7_low_file'])) {
                    $_SESSION['file_url_' . $module_underground_id]['image7_low_file'] = $base_url . ${'image7_low_file_name'};
                }
                if (!empty(${'image7_low_file_name'}) && file_exists(${'image7_low_file_path'})) {
                    $file_exists = true;
                    $file_url = $_SESSION['file_url_' . $module_underground_id]['image7_low_file'];
                } else {
                    $file_exists = false;
                }

                if (file_exists(${'image7_low_log_path'}) && !isset($_SESSION['log_url_' . $module_underground_id]['image7_low_log'])) {
                    $_SESSION['log_url_' . $module_underground_id]['image7_low_log'] = $base_url . ${'image7_low_log_name'};
                }
                ...
                <?php if ($file_exists): ?>
                    <a href="<?php echo htmlspecialchars($file_url); ?>" target="_blank">
                        <img src="pixmaps/icon.png" alt="Image 7_Low File" style="height: 20px;">
                    </a>
                <?php endif; ?>
                <label for="image7_low_file">Image File:</label>
                <input type="file" name="image7_low_file" accept="image/png, image/jpeg, application/pdf">

                <?php if ($log_exists): ?>
                    <a href="<?php echo htmlspecialchars($log_url); ?>" target="_blank">
                        <img src="pixmaps/icon2.png" alt="Image 7_Low Log" style="height: 20px;">
                    </a>
                <?php endif; ?>
